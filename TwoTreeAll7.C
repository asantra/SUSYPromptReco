...
void TwoTreeAll7(){
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoAllRawPfMet7.root","RECREATE");
  ...
  FILE *myfile;
  myfile=fopen("file_All_ReReco7_Sorted.txt", "r");
  char line[50];
  const int linesize = 294982;
  int z1=0;
  ...
  for(int f=0;f<linesize;++f){
    FILE *myfile1;
    myfile1=fopen("file_All_Prompt7_Sorted.txt", "r");
    char line1[50];
