...
void TwoTreeAllCor10(){
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoAllCorPfMet10.root","RECREATE");
  ...
  FILE *myfile;
  myfile=fopen("file_All_ReReco10_Sorted.txt", "r");
  char line[50];
  const int linesize = 342862;
  int z1=0;
  ...
  for(int f=0;f<linesize;++f){
    FILE *myfile1;
    myfile1=fopen("file_All_Prompt10_Sorted.txt", "r");
    char line1[50];
