...
void TwoTreeFake(){
  ...
  TChain b2a("NNtree");
  TChain b2b("NNtree");
  //b1a.Add("PhotonAll_Tree.root");// this file contains pftype01correctedMET
  //b1b.Add("PhotonAll_Tree.root");// this file contains pftype01correctedMET
  b1a.Add("PhotonAll_PfMet_ReReco.root");// this file contains raw pfMET
  b1b.Add("PhotonAll_PfMet_ReReco.root");// this file contains raw pfMET
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoFakeRawPfMet.root","RECREATE");
  ...
  TH1F *h_met_ffSample_RhoReweighted_ErrorFromRho[1000];


  
  TH1F *h_met_ffSample_RhoReweightedPrompt_ErrorFromRho[1000];
  ...
  char *histNameffPrompt       = new char[50];
  char *histtitleffPrompt      = new char[50];

   for (int d=0;d<1000; ++d) {
     sprintf(histNameff, "RhoReweightedffReReco%d",d+1);
     sprintf(histtitleff,"Reweighted ff MET with RHO rereco %d",d+1);
     h_met_ffSample_RhoReweighted_ErrorFromRho[d]=new TH1F(histNameff,histtitleff,200,0.0,200.0);

     sprintf(histNameffPrompt, "RhoReweightedffPrompt%d",d+1);
     sprintf(histtitleffPrompt,"Reweighted ff MET with RHO prompt %d",d+1);
     h_met_ffSample_RhoReweightedPrompt_ErrorFromRho[d]=new TH1F(histNameffPrompt,histtitleffPrompt,200,0.0,200.0);
  ...
  TH1F *FakeRho= (TH1F*)Fnew->Get("FakeRho");
  TH1F *FakeDiEMPt= (TH1F*)Fnew->Get("FakeDiEMPt");
  TH1F *FakeRhoPrompt= (TH1F*)Fnew->Get("FakeRhoPrompt");
  TH1F *FakeDiEMPtPrompt= (TH1F*)Fnew->Get("FakeDiEMPtPrompt");
  ...
  TFile *F2 = new TFile("RandomRatioFF.root","READ");
  TH1F *FFRhoRatio[1000];
  TH1F *FFRhoRatioPrompt[1000];
  ...
  for(int g=0; g<1000; ++g){
    ...
    sprintf(nameGraph,"FakeRhoRandom%d",g+1);
    FFRhoRatio[g] = (TH1F*)F2->Get(nameGraph);

    sprintf(nameGraph2,"FakeRhoRandomPrompt%d",g+1);
    FFRhoRatioPrompt[g] = (TH1F*)F2->Get(nameGraph2);
  }
  
  /////  getting the rereco event list ////////
  FILE *myfile;
  myfile=fopen("file_fake_ReReco.txt", "r");
  char line[50];
  int z1=0;
  ULong64_t ReRecoEventNo[18010]={0};// rereco candidate: 308439, fake: 18010, electron: 2517353, first one 557950
  ULong64_t ReRecoEntry[18010]={0};
  UInt_t ReRecoRunNo[18010]={0};
  ...
  for(int f=0;f<18010;++f){
    FILE *myfile1;
    myfile1=fopen("file_fake_prompt.txt", "r");
    char line1[50];
    ...
    if(same==1){
      ...
      /* reweighting */
      int Bin1=FakeDiEMPt->FindBin(DIEMPT);
      float Weight1=FakeDiEMPt->GetBinContent(Bin1);
      int Bin2=FakeRho->FindBin(rho);
      float Weight2=FakeRho->GetBinContent(Bin2);
      int Bin22=FakeDiEMPtReweightedRho->FindBin(rho);
      float Weight22=FakeDiEMPtReweightedRho->GetBinContent(Bin22);
      ...
      for( int t=0 ; t<1000; ++t){
        float WeightFromRho = FFRhoRatio[t]->GetBinContent(Bin22);
        h_met_ffSample_RhoReweighted_ErrorFromRho[t]->Fill(metet, WeightFromRho);
      }
      ...
      /* reweighting */
      int Bin3=FakeDiEMPtPrompt->FindBin(DIEMPTprompt);
      float Weight3=FakeDiEMPtPrompt->GetBinContent(Bin3);
      int Bin4=FakeRhoPrompt->FindBin(rhoprompt);
      float Weight4=FakeRhoPrompt->GetBinContent(Bin4);
      int Bin44=FakeDiEMPtReweightedRhoPrompt->FindBin(rhoprompt);
      float Weight44=FakeDiEMPtReweightedRhoPrompt->GetBinContent(Bin44);
      ...
      for( int t=0 ; t<1000; ++t){
        float WeightFromRho = FFRhoRatioPrompt[t]->GetBinContent(Bin44);
        h_met_ffSample_RhoReweightedPrompt_ErrorFromRho[t]->Fill(metetprompt, WeightFromRho);
      }
