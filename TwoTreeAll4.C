...
void TwoTreeAll4(){
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoAllRawPfMet4.root","RECREATE");
  ...
  FILE *myfile;
  myfile=fopen("file_All_ReReco4_Sorted.txt", "r");
  char line[50];
  const int linesize = 214667;
  int z1=0;
  ...
  for(int f=0;f<linesize;++f){
    FILE *myfile1;
    myfile1=fopen("file_All_Prompt4_Sorted.txt", "r");
    char line1[50];
