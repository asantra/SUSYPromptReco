...
void TwoTreeAllCor8(){
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoAllCorPfMet8.root","RECREATE");
  ...
  FILE *myfile;
  myfile=fopen("file_All_ReReco8_Sorted.txt", "r");
  char line[50];
  const int linesize = 571721;
  int z1=0;
  ...
  for(int f=0;f<linesize;++f){
    FILE *myfile1;
    myfile1=fopen("file_All_Prompt8_Sorted.txt", "r");
    char line1[50];
