...
void TwoTreeAll5(){
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoAllRawPfMet5.root","RECREATE");
  ...
  FILE *myfile;
  myfile=fopen("file_All_ReReco5_Sorted.txt", "r");
  char line[50];
  const int linesize = 2811;
  int z1=0;
  ...
  for(int f=0;f<linesize;++f){
    FILE *myfile1;
    myfile1=fopen("file_All_Prompt5_Sorted.txt", "r");
    char line1[50];
