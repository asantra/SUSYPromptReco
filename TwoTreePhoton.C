...
void TwoTreePhoton(){
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoPhotonCorPfMet.root","RECREATE");
  ...
  TH1F* h_met_percentdifference1(new TH1F("h_met_percentdifference1","(rereco - prompt)/prompt #slash{E}_{T}, Asymmetric Pt ;#slash{E}_{T} (GeV);Events / GeV", 1000, -50., 50.));
  TH1F* h_met_percentdifference2(new TH1F("h_met_percentdifference2","(rereco - prompt)/rereco #slash{E}_{T}, Asymmetric Pt ;#slash{E}_{T} (GeV);Events / GeV", 1000, -50., 50.));
  ...
  FILE *myfile;
  myfile=fopen("file_photon_ReReco.txt", "r");
  char line[50];
  int z1=0;
  const int linesize = 308439; // rereco candidate: 308439, fake: 18010, electron: 2517353, first one 557950
  ULong64_t ReRecoEventNo[linesize]={0};
  ...
  for(int f=0;f<linesize;++f){
    FILE *myfile1;
    myfile1=fopen("file_photon_prompt.txt", "r");
    char line1[50];
