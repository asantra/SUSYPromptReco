...
void TwoTreeAll1(){
  ...
  //// histograms ////
  TFile *FF = new TFile("PromptRecoAllRawPfMet1.root","RECREATE");
  ...
  FILE *myfile;
  myfile=fopen("file_All_ReReco1_Sorted.txt", "r");
  char line[50];
  const int linesize = 399764;
  int z1=0;
  ...
  for(int f=0;f<linesize;++f){
    FILE *myfile1;
    myfile1=fopen("file_All_Prompt1_Sorted.txt", "r");
    char line1[50];
